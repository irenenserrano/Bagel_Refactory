...
public class Order {
	...
	/**
	 * Generates a printed receipt containing the contents, discounts, and total
	 * final price
	 *
	...
	public String generateReceipt() {
		...
		for (Bag bag : bags) {
			...
			s += bag.getBagel().getType() + "\n" + "\tquantity: " + bag.getQuantity() + "\n" + "\tprice each: $"
					+ bag.getPerBagelPrice() + "\n" + "\total: $" + "\ttotal: " + bag.getTotalPrice() + "\n";
			if (bag.getTotalPrice()
					.compareTo(bag.getPerBagelPrice().multiply(BigDecimal.valueOf(bag.getQuantity()))) < 0) {
				s += "\tYou saved $" + (bag.getPerBagelPrice().multiply(BigDecimal.valueOf(bag.getQuantity()))
						.subtract(bag.getTotalPrice())) + " through our volume discount program.\n";
			}
		}
		s += "TOTAL: $" + total + "\n";

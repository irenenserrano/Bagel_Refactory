...
class OrderTest {
    ...
    private static Arguments makeBagArguments(Bagel.Type type, int quantity, BigDecimal price) {
        return Arguments.of(new Bag(new Bagel(type), quantity), price);
    }// end makeBagArguments

    private static Arguments makeOrderArguments(Order order) {
        return Arguments.of(new Order(Bag bags));
    ...
    class OrderArgumentsProvider_OneBagEachCategory implements ArgumentsProvider {
        ...
        public Stream<? extends Arguments> provideArguments(ExtensionContext arg0)
                throws Exception {
            blueberry.markDown();
            return Stream.of(Arguments.of(new Order(bag3plain), 1.50),
                    Arguments.of(new Order(bag6plain), 2.85),
                    Arguments.of(new Order(bag13plain), 5.70),
                    Arguments.of(new Order(bag3asiago), 2.10),
                    Arguments.of(new Order(bag6asiago), 3.99),
                    Arguments.of(new Order(bag13asiago), 8.65),
